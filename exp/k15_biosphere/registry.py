"""Axis registry for the K15 biosphere rewrite (ticket 0041; spec B9 §2).

Port of the settled k13 registry (``exp/k13_treegen/registry.py``), adapted
and self-contained.  Every axis (a morphometric knob or a flat vocabulary
axis) is one ``AxisSpec``.  The ``Registry`` is the validated,
consumer-ready view of the schema — the derived-view assembler, the mass
hook, couplings, and naming all dispatch on per-axis metadata held here.
**If a future consumer needs new per-axis data, it is added to
``AxisSpec`` once — never per module.**

Three-tier taxonomy (owner ruling): ``invariant`` (plan topology — change
it and you changed the plan, not the species) → ``steady`` (proportions,
small drift) → ``labile`` (ornaments, color; large drift).

Vary-by-default with a clade-steady blacklist (the v1 freeze-bug fix,
stated as a schema rule): every axis that is neither ``invariant`` nor
...
Size convention (B1 v0.3 §1): exactly one size axis — ``unit=mass`` for
fauna, ``unit=length`` (height_m) for flora.  The ``unit`` lint enforces
one non-dimensionless axis registry-wide.

Consumers: a closed vocabulary (``VALID_CONSUMERS``) — every axis names
who reads it (rent audit).  Ported k13's vocabulary unchanged and EXTENDED
with the future L2 biomass/occupancy consumers (spec B9 §2); the real
flora content's consumer names are all kept.
"""
...
class Tier(str, Enum):
    INVARIANT = "invariant"   # plan topology; the sampler must not touch
    STEADY = "steady"         # proportions / limb indices; small drift
    LABILE = "labile"         # ornaments, color; large drift
...
class MutationKind(str, Enum):
    ...
    LOG_GAUSSIAN = "log_gaussian"  # multiplicative on a positive axis (size)
...
class Unit(str, Enum):
    ...
    LENGTH = "length"              # flora size axis (height_m)
...
# k13 vocabulary, plus the L2 consumers the derived view's occupancy and
# biomass-density layers will read (spec B9 §2: extend, keep every name
# the real flora content already uses).
VALID_CONSUMERS = {"stress", "drift", "runaway", "id", "name", "tell",
                   "pop", "draw", "biomass", "occupancy"}
...
class AxisSpec:
    ...
    salience: float = 0.0                          # epithet / part picking
    ...
    adapt_weight: float | None = None   # 0=decorative .. 1=adaptive
    # RESERVED (future rounds layer): functional effect vector, e.g.
    # {thermal: 0.8, camouflage: -0.6, warning: 0.9}. Parsed and shape-
    # validated, consumed by NOTHING yet — the hook for "knobs give stat
    # bonuses/debuffs" (B9 keeps the k13 reservation).
...
class PlanSpec:
    ...
    permission table) + slot list (string enums) + medium."""
    ...
    phylum: str = ""            # taxonomy: phylum anchor
...
class Registry:
    ...
    def salience_order(self, plan: str | None = None) -> list[AxisSpec]:
        """Axes ordered by descending salience (epithet selection)."""
    ...
    def grammar_index(self) -> dict[GrammarRole, list[str]]:
        """grammar_role -> axis names (description template slots)."""
    ...
    def validate(self, coupling_ids: set[str] | None = None) -> list[str]:
        ...
        # (height_m) for flora (the K15 registry is kingdom-neutral).
