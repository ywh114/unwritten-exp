# K15 sim-diff engine (flora rounds) — build spec v0.2

2026-08-01. v0.2 folds the critic review (23 findings): the population
...
s_dens   = DENS_C · D(c) / K(c)      (the B5 capacity-denominator form:
           dimensionless, self-limiting, implements competition via
           productivity; clip to DENS_CAP; K(c) ≤ K_EPS with D > 0 →
           s_dens = DENS_CAP)
...
GATE: establishment only where f_hab ≥ EST_F_MIN (cal, default 0.35)
```
...
2. Seed cells with F_worst ≥ GENESIS_F (cal, default 0.6) at
...
8. **Performance**: genesis (35 evals) + 20 rounds ≤ 60 s wall; cache
   ≤ REDUCED_CACHE_MB (3.7 MB) per live instance.
...
| EST_F_MIN / EST_N0 | 7.3 | 0.35 / 0.05 | gate floor / founder density |
| SEEDBANK_KEEP | 7.3 | 0.5 | persistent rain carryover |
| GENESIS_F / GENESIS_N0 | 10 | 0.6 / 0.2 | genesis threshold/density |
