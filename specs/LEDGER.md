...
| `k15-simdiff-engine.md` v0.2 | proposal | amends/feeds B5 + flora RFC; the K15 engine build spec — genesis rain, dispersal (channel pmf + stress-gated pull + jump), population update (B5 rates, birth suppressed by negative stress only, per-year rate compounding fixed), TreeAuthority + reflog, merge/split rulings | spec written 2026-08-01 (v0.2 folds critic review, 23 findings — 5 blockers incl. space-blind merge fix and K(c) equilibrium); implementation pending |
...
| K15 | sim-diff: stress adapter + dispersal + cover + rounds (flora P7–P9, fauna A-rounds); consumes K11 + K14 products + K13 trees; shared stress math in `kernel/stress/` | B5, build plan decision 2 | **spec-written** (`specs/k15-simdiff-engine.md` v0.2, 2026-08-01); landed: `kernel/stress/` + req_flora view contract + stress adapter + FloraSim adapter (dispersal view threaded incl. jump_rate, commit 037a2f3); pending: engine (genesis/dispersal/population/engine) + `authority.py` TreeAuthority + reflog (spec §9) |
