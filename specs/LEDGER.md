...
| `k15-simdiff-engine.md` v0.4 | proposal | amends/feeds B5 + flora RFC; the K15 engine build spec — genesis rain, dispersal (channel pmf + stress-gated pull + jump), population update (B5 rates, birth suppressed by negative stress only, per-year rate compounding fixed), TreeAuthority + reflog, merge/split rulings; v0.3: substrate capacity split K_L = K·U (best-of-class suitability, split capacity), growing-month worst-month, settled GENESIS_F 0.5 / EST_F_MIN 0.3, content authoring conventions; v0.4: rule B+ founding/differentiation — gene-flow-keyed minting (contiguous spill joins, jump mints with vicinity absorption, sustained remote landings join through a VERDICT gate TH = DIFF_D·(1+MOB_K·mobility)), failed-gate cells incubate as divergent sub-ranges and split only at DIFF_MIN_CELLS (sliver suppression), §8 rain-bridge connectivity | spec written 2026-08-01 (v0.2 folds critic review, 23 findings — 5 blockers incl. space-blind merge fix and K(c) equilibrium); v0.3 same day folds the stat-settling pass (statpass.py, 35 presets × seeds 1-3: 0/35 NO_RANGE seed 1, 4/35 seed 2, 2/35 seed 3, all gaps genuine world content); v0.4 same day: rule B+ knobs calibrated on seed-1 round-0 founding data (old rule 1173 mints, median 1 cell → DIFF_D 0.2 / MOB_K 1.0 / DIFF_MIN_CELLS 32); engine implemented per v0.4 |
