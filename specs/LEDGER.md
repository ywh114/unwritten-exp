...
| `k15-simdiff-engine.md` v0.3 | proposal | amends/feeds B5 + flora RFC; the K15 engine build spec — genesis rain, dispersal (channel pmf + stress-gated pull + jump), population update (B5 rates, birth suppressed by negative stress only, per-year rate compounding fixed), TreeAuthority + reflog, merge/split rulings; v0.3: substrate capacity split K_L = K·U (best-of-class suitability, split capacity), growing-month worst-month, settled GENESIS_F 0.5 / EST_F_MIN 0.3, content authoring conventions | spec written 2026-08-01 (v0.2 folds critic review, 23 findings — 5 blockers incl. space-blind merge fix and K(c) equilibrium); v0.3 same day folds the stat-settling pass (statpass.py, 35 presets × seeds 1-3: 0/35 NO_RANGE seed 1, 4/35 seed 2, 2/35 seed 3, all gaps genuine world content); implementation pending |
...
| K15 | sim-diff: stress adapter + dispersal + cover + rounds (flora P7–P9, fauna A-rounds); consumes K11 + K14 products + K13 trees; shared stress math in `kernel/stress/` | B5, build plan decision 2 | **spec-written** (`specs/k15-simdiff-engine.md` v0.3, 2026-08-01); landed: `kernel/stress/` + req_flora view contract + stress adapter (best-of-class substrate + capacity split, growing-season dormancy for climate AND water terms, wet-land fresh_availability) + FloraSim adapter + `statpass.py` calibration harness; content settled through stat-pass E (moisture niches re-anchored to the c_P scale, marine pH to 7.9, K14 ground table deepened); pending: engine (genesis/dispersal/population/engine) + `authority.py` TreeAuthority + reflog (spec §9) |
